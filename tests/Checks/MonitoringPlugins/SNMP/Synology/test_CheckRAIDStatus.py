import pytest

from icinga2confgen.Checks.MonitoringPlugins.SNMP.Synology.CheckRAIDStatus import CheckRAIDStatus
from icinga2confgen.Commands.MonitoringPlugins.SNMP.Synology.RAIDStatusCommand import RAIDStatusCommand
from icinga2confgen.Groups.ServiceGroup import ServiceGroup
from tests.BaseCheckSNMPTest import BaseCheckSNMPTest


class TestCheckRAIDStatus(BaseCheckSNMPTest):

    def get_instance_class(self):
        return CheckRAIDStatus

    def get_command_class(self):
        return RAIDStatusCommand

    def get_default_service_groups(self):
        return [
            ServiceGroup.create('synology'),
            ServiceGroup.create('system_health'),
            ServiceGroup.create('disk'),
            ServiceGroup.create('snmp')
        ]

    def create_instance(self, force=False):
        instance = BaseCheckSNMPTest.create_instance(self, force)
        instance.set_raids(99)
        return instance

    def test_validate_raise_exception_on_missing_raids(self):
        instance = BaseCheckSNMPTest.create_instance(self)
        instance.set_host('host')
        instance.set_username('user')
        instance.set_password('pwd')
        self.validate_snapshot = False
        with pytest.raises(Exception) as excinfo:
            instance.validate()

    def test_get_right_raids(self):
        instance = self.create_instance()
        instance.set_raids(5)

        assert 5 == instance.get_raids()

    def test_get_right_warning(self):
        instance = self.create_instance()
        instance.set_warning(44)

        assert 44 == instance.get_warning()

    def test_get_right_critical(self):
        instance = self.create_instance()
        instance.set_critical(55)

        assert 55 == instance.get_critical()
