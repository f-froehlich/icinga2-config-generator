...
T = typing.TypeVar('T', bound='CheckBatteryRemainingRuntime')
...
class CheckBatteryRemainingRuntime(CheckSNMP):
    ...
    def create(id: str, force_create: bool = False) -> T:
        ...
        if None is check:
            check = CheckBatteryRemainingRuntime(id)
            ConfigBuilder.add_check(id, check)
        elif not isinstance(check, CheckBatteryRemainingRuntime):
            raise Exception('Id must be for an instance of CheckBatteryRemainingRuntime but other instance is returned')

        if None is ConfigBuilder.get_command('monitoring_plugins_snmp_powernet_mib_battery_remaining_runtime'):
            BatteryRemainingRuntimeCommand.create('monitoring_plugins_snmp_powernet_mib_battery_remaining_runtime')
