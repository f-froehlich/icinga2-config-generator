...
T = typing.TypeVar('T', bound='CheckSpaceIO')
...
class CheckSpaceIO(CheckSNMP):
    ...
    def set_volume(self: T, number: int) -> T:
        self.__volume = number
        return self

    def get_volume(self: T) -> Union[int, None]:
    ...
    def create(id: str, force_create: bool = False) -> T:
        ...
        if None is check:
            check = CheckSpaceIO(id)
            ConfigBuilder.add_check(id, check)
        elif not isinstance(check, CheckSpaceIO):
            raise Exception('Id must be for an instance of CheckSpaceIO but other instance is returned')

        if None is ConfigBuilder.get_command('monitoring_plugins_snmp_synology_space_io'):
            SpaceIOCommand.create('monitoring_plugins_snmp_synology_space_io')
