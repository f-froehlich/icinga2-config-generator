...
T = typing.TypeVar('T', bound='GPUInfoCommand')


class GPUInfoCommand(SNMPCommand):
    ...
    def create(id: str, force_create: bool = False) -> T:
        ...
        if None is command:
            command = GPUInfoCommand(id)
            ConfigBuilder.add_command(id, command)
        elif not isinstance(command, GPUInfoCommand):
            raise Exception('Id must be for an instance of GPUInfoCommand but other instance is returned')

        return command

    def get_command(self: T) -> str:
        return 'snmp/Synology/check_gpu_info.py'
