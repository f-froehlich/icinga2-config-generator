...
T = typing.TypeVar('T', bound='DiskTemperatureCommand')


class DiskTemperatureCommand(SNMPCommand):
    ...
    def create(id: str, force_create: bool = False):
        ...
        if None is command:
            command = DiskTemperatureCommand(id)
            ConfigBuilder.add_command(id, command)
        elif not isinstance(command, DiskTemperatureCommand):
            raise Exception('Id must be for an instance of DiskTemperatureCommand but other instance is returned')

        return command

    def get_command(self: T) -> str:
        return 'snmp/Synology/check_disk_temperature.py'
