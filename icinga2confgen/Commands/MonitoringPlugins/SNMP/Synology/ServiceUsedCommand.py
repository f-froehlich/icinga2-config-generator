...
class ServiceUsedCommand(SNMPCommand):
    ...
    def create(id: str, force_create: bool = False):
        ...
        if None is command:
            command = ServiceUsedCommand(id)
            ConfigBuilder.add_command(id, command)
        elif not isinstance(command, ServiceUsedCommand):
            raise Exception('Id must be for an instance of ServiceUsedCommand but other instance is returned')

        return command

    def get_command(self: T) -> str:
        return 'snmp/Synology/check_service_used.py'
